...
class Minesweeper(commands.Cog):
    ...
    async def minesweeper(self, ctx):
                ...
                for x in msg:
                    ...
                    if cn > 10 or cn <= 0:
                        ...
                        if answer_board[rn][cn] == ":bomb:":
                            await ctx.send(
                                f"LOL!  {ctx.author.mention} you stumbled across a bomb!  The answer board has been posted above."
                            )
                            ...
                            if(showing_board[rn][cn] != answer_board[rn][cn]):
                                rem -= 1
        ...
        if (ded == 0):
            await ctx.send("Victory! 15k have been deposited")
            await bank.deposit_credits(ctx.author, 15000)
